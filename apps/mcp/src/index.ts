...
export function registerTools(server: McpServer, dependencies: ToolDependencies): void {
  ...
  server.registerTool(
    "build_production_set",
    {
      ...
      description:
        "Builds a Bates-stamped discovery production package with upload files, production index, manifest, checksums, optional volumes, optional combined PDF, and an optional litigation load file (production.dat, includeLoadFiles). Detects duplicate sources and reports them (duplicateGroups/duplicateCount); duplicateHandling controls whether every occurrence is produced (default) or only the first. Per-source status marks a document withhold (its own bytes are never produced; requires privilegeAsserted) or produce-redacted (produced normally -- this tool applies no redaction itself); either writes a draft-privilege-log.csv (withheldCount/redactedCount/privilegeLogLocation) that is NOT ready to serve without manual completion. How a withheld document's place is represented is controlled by withheldHandling: the default \"slip-sheet\" puts a Bates-stamped placeholder in its position (appearing in upload/, the index, and the DAT, consuming one Bates number), while \"omit\" leaves it out entirely, consuming none.",
