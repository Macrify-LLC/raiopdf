...
| **Production sets** | Build a Bates-numbered discovery production from a document set — confidentiality designations, index files, volume splits, an optional Relativity-compatible DAT load file, one package out. Withhold a document or flag it as produced with redactions (applied upstream, not by RaioPDF) and it writes a draft privilege log — a draft only, with the Date/Doc Type/Author/Recipients columns always left for you to fill in by hand |
