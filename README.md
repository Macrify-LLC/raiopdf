...
| **Word ↔ PDF** *(experimental)* | Import a Word document (with a tracked-changes check first), add Word documents alongside PDFs where supported, or export a PDF back to editable Word. Runs through your own installed, signed-in copy of Microsoft Word. On macOS, allow RaioPDF under **System Settings > Privacy & Security > Automation** if asked; if you deny it, enable it there before retrying because retrying does not show the prompt again. Formatting is approximate; check the result. |
