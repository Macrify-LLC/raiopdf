...
| `build_production_set` | Build a Bates-numbered discovery production from a document set: confidentiality designations (optionally restricted to a per-source page range via `designationPages`, e.g. `"1-3,7"`), configurable stamp placement and shared font size (`batesPlacement`/`designationPlacement`/`stampFontSizePt` — Bates and the designation can't share a page edge), index files, volume splits. Detects duplicate sources (matching content hash) and reports them as `duplicateGroups`/`duplicateCount`; `duplicateHandling` controls the outcome — `"produce-all"` (default) stamps and produces every occurrence with its own Bates range, `"produce-once"` produces only the first occurrence and omits the rest, consuming no Bates numbers for the omitted copies. Optionally continues the same Bates series from a prior production package (`continueFrom`, manifest-verified), with an explicit-reason override (`continuationOverride`) for a deliberate gap or digit-width change. `includeLoadFiles` (default false) writes a litigation load file (`production.dat`, "Relativity-compatible Concordance DAT defaults" — see [`docs/PRODUCTION-SETS.md`](PRODUCTION-SETS.md)) at the package root; per-source `custodian` populates its CUSTODIAN column. Per-source `status` — `"produce"` (default), `"produce-redacted"`, or `"withhold"` — marks a document withheld or produced with redactions applied upstream (produced NORMALLY — this tool performs no redaction itself; use `redact_terms` first); withholding requires `privilegeAsserted`. `withheldHandling` (default `"slip-sheet"`) controls how a withheld source's canonical occurrence appears: `"slip-sheet"` takes its place with a generated, Bates-stamped one-page placeholder ("DOCUMENT WITHHELD," the privilege asserted, an optional basis) consuming exactly one Bates number in position (`slipSheetCount` reports how many); `"omit"` excludes it entirely before Bates numbers are assigned, consuming none — if every source is withheld under `"omit"`, the build still succeeds as an audit-only package with an empty upload/ and an unchanged Bates sequence. Either status writes a draft `draft-privilege-log.csv` at the package root (`withheldCount`/`redactedCount`/`privilegeLogLocation`; `includeFilenameInPrivilegeLog`, default true; a `Bates` column shows the range a produced row consumed, blank for an omitted withheld row) — a byte-identical duplicate group with conflicting statuses is a pre-output error, and a uniformly-withheld duplicate group produces exactly one slip sheet (or one omission) and one log row. The log is a draft only: `Date`/`DocType`/`Author`/`Recipients` are always blank for manual completion, and it is not ready to serve without review — see [`docs/PRODUCTION-SETS.md`](PRODUCTION-SETS.md). |
